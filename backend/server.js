...
app.post('/auth/register', async(req, res) => {
    ...
    const userExist = await db.findUserByEmail(email);
    if (userExist) return res.status(400).json({msg: 'Email já existe'});
...
app.post('/auth/login', async(req, res) => {
    ...
    if(!valid) return req.status(401).json({ msg: 'Senha inválida'});

    const token = jwt.sign(
        {id: user.id},
        process.env.JWT_SELECT,
        {expiresIn: '1d'}
    );
    res.json({ token, user});
});
